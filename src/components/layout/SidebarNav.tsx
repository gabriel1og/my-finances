  ...
  return (
    <nav className="flex flex-col">
      {NAV_SECTIONS.map((section) => (
        <div key={section.key} className="mb-1 flex flex-col">
          {/* O rótulo da seção some com o menu recolhido: em 64px ele
              viraria três letras cortadas. A separação continua legível pelo
              espaço entre os grupos. */}
          {collapsed ? (
            <span className="mx-auto my-2 h-px w-6 bg-border" aria-hidden />
          ) : (
            <span className="label-caps px-5 py-2">{section.label}</span>
          )}
